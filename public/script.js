    ...
    _drSesiList.forEach((s, sIdx) => {
      ...
      html += `
        <div style="display:flex;align-items:center;gap:10px;padding:10px 16px 10px;
          ...
          ${avatarHtml}
          <div style="flex:1;min-width:0;">
            <div style="font-size:17px;font-weight:800;color:#222;letter-spacing:.3px;">
        ...
        s.breaks.forEach((b, bIdx) => {
          ...
          html += `
            <div style="display:flex;align-items:center;gap:10px;padding:10px 16px;
              ...
              ${avatarHtml}
              <div style="flex:1;min-width:0;">
                <div style="font-size:17px;font-weight:800;color:#222;letter-spacing:.3px;">${bsMulai}</div>
          ...
          if (b.end) {
            html += `
              <div style="display:flex;align-items:center;gap:10px;padding:10px 16px;
                ...
                ${avatarHtml}
                <div style="flex:1;min-width:0;">
                  <div style="font-size:17px;font-weight:800;color:#222;letter-spacing:.3px;">${bsSelesai}</div>
                  <div style="margin-top:2px;">
                    <span style="font-size:10px;background:#e8f5e9;color:#2e7d32;border-radius:4px;
      ...
      html += `
        <div style="display:flex;align-items:center;gap:10px;padding:10px 16px;
          ...
          ${avatarHtml}
          <div style="flex:1;min-width:0;">
            <div style="font-size:17px;font-weight:800;color:${isAktif?"#f9a825":"#222"};letter-spacing:.3px;">
...
function _tsPickerHTML(idPrefix, initJam, initMenit) {
  ...
  return `
    <!-- Tampilan jam besar -->
    <div style="padding:6px 16px 4px;border-bottom:1px solid #f0f2f5;
                display:flex;align-items:center;gap:8px;">
      <span id="${idPrefix}-display"
        style="font-size:20px;font-weight:800;color:#e65100;font-variant-numeric:tabular-nums;
    ...
    <!-- Drum scroll -->
    <div style="display:flex;height:100px;overflow:hidden;position:relative;">
      <!-- Garis highlight tengah -->
      <div style="position:absolute;left:0;right:0;top:50%;transform:translateY(-50%);
                  height:30px;background:rgba(245,124,0,.08);pointer-events:none;z-index:1;
        ...
        ${Array.from({length:24},(_,n)=>`
          <div data-v="${n}"
            style="height:30px;display:flex;align-items:center;justify-content:center;
                   font-size:15px;font-weight:700;scroll-snap-align:center;cursor:pointer;
...
function _tsPickerScrollH(prefix, el) {
  ...
  _tsPickerScrollTimer[prefix+"h"] = setTimeout(() => {
    const v = Math.round(el.scrollTop / 30);
    el.scrollTop = v * 30; // snap
...
function _tsPickerInit(prefix, jamStr) {
  ...
  requestAnimationFrame(() => {
    const colH = document.getElementById(`${prefix}-col-h`);
    const colM = document.getElementById(`${prefix}-col-m`);
    if (colH) { colH.scrollTop = h * 30; }
    if (colM) { colM.scrollTop = m * 30; }
    _tsPickerUpdate(prefix);
  });
}
...
function _tsPickerGetVal(prefix) {
  ...
  const colM = document.getElementById(`${prefix}-col-m`);
  if (!colH || !colM) return "";
  const h = Math.min(23, Math.max(0, Math.round(colH.scrollTop / 30)));
  const m = Math.min(59, Math.max(0, Math.round(colM.scrollTop / 30)));
      ...
      <div style="border:1.5px solid #e8ecf0;border-radius:14px;overflow:hidden;margin-bottom:12px;background:white;">
        ...
        <div style="border-bottom:1px solid #f0f2f5;">
          ${isIstirahat ? `
            ...
            <div style="display:flex;gap:0;">
              ...
              <div style="flex:1;border-right:1px solid #f0f2f5;">
                <div style="padding:6px 12px 3px;font-size:10px;font-weight:700;color:var(--muted);text-transform:uppercase;">Mulai</div>
                <div style="padding:2px 12px 6px;display:flex;align-items:center;gap:6px;border-bottom:1px solid #f0f2f5;">
                  <span id="${prefix}-disp-a" style="font-size:16px;font-weight:800;color:#e65100;font-variant-numeric:tabular-nums;">
                    ${String(eH).padStart(2,"0")}:${String(eM).padStart(2,"0")}
                  </span>
                </div>
                <div style="display:flex;height:90px;overflow:hidden;position:relative;">
                  <div style="position:absolute;left:0;right:0;top:50%;transform:translateY(-50%);height:28px;
                  ...
                  <div id="${prefix}-col-ah" class="ts-dr-col"
                    ...
                    ${Array.from({length:24},(_,n)=>`<div data-v="${n}" onclick="_tsPickerClickH('${prefix}-a',${n})"
                      style="height:28px;display:flex;align-items:center;justify-content:center;font-size:13px;
              ...
              <!-- Picker selesai -->
              <div style="flex:1;">
                <div style="padding:6px 12px 3px;font-size:10px;font-weight:700;color:var(--muted);text-transform:uppercase;">Selesai</div>
                <div style="padding:2px 12px 6px;display:flex;align-items:center;gap:6px;border-bottom:1px solid #f0f2f5;">
                  <span id="${prefix}-disp-b" style="font-size:16px;font-weight:800;color:#e65100;font-variant-numeric:tabular-nums;">
          ...
          ` : `
            ...
            <div style="padding:6px 14px 4px;display:flex;align-items:center;gap:8px;border-bottom:1px solid #f0f2f5;">
              <span id="${prefix}-disp-a" style="font-size:20px;font-weight:800;color:#e65100;
            ...
            <div style="display:flex;height:100px;overflow:hidden;position:relative;">
              ...
              <div id="${prefix}-col-ah" class="ts-dr-col"
                ...
                <div style="height:35px;"></div>
                ${Array.from({length:24},(_,n)=>`<div data-v="${n}" onclick="_tsPickerClickH('${prefix}-a',${n})"
                  style="height:30px;display:flex;align-items:center;justify-content:center;font-size:15px;
                         font-weight:${n===eH?900:700};color:${n===eH?"#e65100":"var(--text)"};
                         scroll-snap-align:center;cursor:pointer;" class="ts-pk-h ${n===eH?"ts-picker-selected":""}">
                  ${String(n).padStart(2,"0")}</div>`).join("")}
                <div style="height:35px;"></div>
  ...
  requestAnimationFrame(() => {
    _drEntries.forEach((e, i) => {
      ...
      if (isIstirahat) {
        ...
        if (colBH) colBH.scrollTop = sH * 28;
        if (colBM) colBM.scrollTop = sM * 28;
      }
...
async function tsSimpanTambahEntri() {
  ...
  _drEntries.forEach((e, i) => {
    const prefix = `tse-${i}`;
    const itemH = e.tab === "istirahat" ? 28 : 30;
    const colAH = document.getElementById(`${prefix}-col-ah`);
    const colAM = document.getElementById(`${prefix}-col-am`);
    if (colAH && colAM) {
    ...
    if (e.tab === "istirahat") {
      ...
      const colBM = document.getElementById(`${prefix}-col-bm`);
      if (colBH && colBM) {
        const h2 = Math.min(23, Math.max(0, Math.round(colBH.scrollTop / 28)));
        const m2 = Math.min(59, Math.max(0, Math.round(colBM.scrollTop / 28)));
